...
def sample_images(generator,Tensor,imgs,path):
    """
    ...
    cv2.imwrite("generate.png" ,255*fake_B[0].squeeze(0).cpu().swapaxes(0,2).swapaxes(0,1).detach().numpy())
    result = cv2.imread("generate.png")
    result = cv2.resize(result, sample.shape[1::-1]) 
    cv2.imwrite("resizedresult.png", result)
def process(opt,file_path):
    ...
    PIL.Image.fromarray((tenOutput.clip(0.0, 1.0).detach().numpy().transpose(1, 2, 0)[:, :, 0] * 255.0).astype(numpy.uint8)).save(arguments_strOut)
...
def main(path):
    ...
    generator.load_state_dict(torch.load("generator_45_canny.pth", map_location=torch.device('cpu')))
    ...
    sample_images(generator,Tensor,frame,path) #输入pix2pix模型求解


if __name__ == "__main__":
    path = "test_pic/7.jpg" # 要处理的图片
    main(path)
